#!/usr/bin/env python3
"""Deterministic Issue #350/#352 runtime-v3 control-plane transition.
...
TRACKED_RUNTIME_FILES = (
    "scripts/issue350_352_runtime_v3_transition.py",
    "specs/350-opensearch-memory-headroom/runtime-v3-transition.md",
)
...
class Protocol:
    ...
    def validate_local_checkout(self, deadline):
        ...
        if parent not in {PRODUCT_BASE_COMMIT, PRODUCT_BASE_COMMIT + "\n"}:
            raise TransitionError("runtime commit is not based directly on the reviewed Issue #352 product base")
    ...
    def preflight(self, deadline):
        ...
        for name, application in applications.items():
            ...
            try:
                ...
                valid = False
            required = name in {"root-app", "argocd", "opensearch"}
    ...
    def final_passes(self, applications, deadline):
        try:
            ...
            if not self.operation_passes(os_app, self.baseline["operations"]["opensearch"], ["3.7.0", self.config.runtime_commit, self.config.old_commit]):
                return False
            for name, application in applications.items():
                if (name not in {"root-app", "opensearch"} and
                        operation_identity(application) != self.baseline["operations"][name]):
                    return False
