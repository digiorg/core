...
|-------------|-----|---------------|
| Local (KinD) | http://digiorg.local/gitea | Admin account (see below) |

**Initial Setup:**
1. Sign in with admin credentials (retrieve password: `kubectl get secret gitea-admin-secret -n gitea -o jsonpath='{.data.password}' | base64 -d`)
