#!/usr/bin/env python3
"""Behavioral contracts for the offline-testable Issue #350 transition."""
...
SCRIPT = ROOT / "scripts" / "issue350_runtime_v2_transition.py"
RUNBOOK = ROOT / "specs" / "350-opensearch-memory-headroom" / "runtime-v2-transition.md"
...
NEW_TAG = "issue350-runtime-v2-20260902T063718Z"
    ...
    def run(self, argv, timeout):
        ...
        if verb == "get" and tail[0] == "horizontalpodautoscalers.autoscaling":
            if self.controller["spec"]["replicas"] == 0 and self.hpa_after_barrier:
                return self._json({"items": [{"metadata": {"name": "late", "namespace": "argocd"}, "spec": {"scaleTargetRef": {"apiVersion": "APPS/v1", "kind": "statefulset", "name": "argocd-application-controller"}}}]})
            return self._json({"items": deepcopy(self.hpas)})
...
class RunbookContractTest(unittest.TestCase):
    def test_runbook_has_placeholder_free_mutually_exclusive_clean_bootstrap_path(self):
        ...
        self.assertIn("kind delete cluster --name digiorg-core-dev", clean)
        self.assertIn('test -z "$(kind get clusters | grep -Fx digiorg-core-dev || true)"', clean)
        self.assertIn("git clone --no-checkout https://github.com/digiorg/core.git /home/simonremote/issue350-clean-bootstrap", clean)
        self.assertIn('git checkout --detach "refs/tags/issue350-runtime-v2-20260902T063718Z^{}"', clean)
        self.assertIn("nu scripts/local-setup.nu up", clean)
        self.assertIn("--mode clean-bootstrap-accept", clean)
        self.assertIn("--kubeconfig /home/simonremote/issue350-clean-bootstrap/kubeconfig-local.yaml", clean)
        self.assertIn("--evidence /home/simonremote/issue350-clean-bootstrap-accept.jsonl", clean)
        self.assertNotIn("/secure/", clean)
        self.assertNotIn("<", clean)
        self.assertNotIn("--current-primary-index", clean)
        self.assertNotIn("--representative-log-index", clean)
    ...
    def test_runbook_declares_executable_authority_and_exact_retained_window(self):
        text = RUNBOOK.read_text(encoding="utf-8")
        self.assertIn("`scripts/issue350_runtime_v2_transition.py` is authoritative", text)
        self.assertNotIn("`scripts/issue350_runtime_v2_transition.py is authoritative`", text)
        for argument in (
...
class SecurityTest(Harness):
    def test_local_checkout_integrity_git_checks_precede_kubernetes(self):
        ...
        self.assertIn(["git", "ls-tree", "--name-only", "HEAD", "--", "scripts/issue350_runtime_v2_transition.py"], git_commands)
        self.assertIn(["git", "ls-tree", "--name-only", "HEAD", "--", "specs/350-opensearch-memory-headroom/runtime-v2-transition.md"], git_commands)
    ...
    def test_evidence_has_auditable_phase_facts_and_rfc3339_time(self):
        ...
        self.assertEqual(by_event["control-plane-closed"]["source_counts"], {"old": 29, "v2": 3})
...
class TransitionBehaviorTest(Harness):
    ...
    def test_hpa_and_degraded_controller_fail_before_mutation(self):
        fake = StatefulFakeKubectl()
        fake.hpas = [{"metadata": {"name": "bad", "namespace": "argocd"}, "spec": {"scaleTargetRef": {"apiVersion": "apps/v1", "kind": "StatefulSet", "name": "argocd-application-controller"}}}]
