...
class SourceContractTest(unittest.TestCase):
    ...
    def test_final_source_graph_matches_all_reviewed_application_manifests(self):
        ...
        self.assertEqual(sum(identity[-1] == NEW_TAG for identity in core), 5)
        self.assertEqual(sum(identity[-1] == OLD_TAG for identity in core), 27)
        self.assertFalse(any(identity[-1] == PREVIOUS_TAG for identity in core))
...
class TransitionBehaviorTest(Harness):
    ...
    def test_exact_kyverno_preflight_exception_is_required(self):
        cases = (
            ...
            lambda app: app["status"]["resources"][0].update({"status": "Synced"}),
            lambda app: app["status"]["resources"][0].update({"kind": "Deployment"}),
        )
        for mutate in cases:
            with self.subTest(mutate=mutate):
                if self.evidence.exists():
