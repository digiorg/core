...
│   ├── opensearch.yaml         # Log and Trace Storage (Wave 0)
...
| 0 | cert-manager, postgresql, nats, external-secrets, opensearch | TLS certificates + shared database + messaging + secret management + log/trace storage |
...
These services have no platform dependencies and are deployed first:
- **External Secrets Operator** — enables secret management from external providers (Azure KeyVault, AWS Secrets Manager)
- **OpenSearch** — provides log and trace storage backend for Jaeger and Grafana
...
These services require OpenSearch as a storage backend:
- **Jaeger** — uses OpenSearch as the trace storage backend
...
| code-quality | sonarqube-saml-secret | Keycloak IdP certificate for SAML authentication |
