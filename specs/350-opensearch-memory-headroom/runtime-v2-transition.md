# Issue #350 Runtime v2 Transition Contract

## Status, authority, and release gate

This candidate does not authorize rollout. The source graph remains unauthorized until the runtime commit exists, the annotated tag and both remote refs resolve to that exact SHA, exact-SHA CI passes, independent review covers that exact snapshot, and there is separate Chris approval for the retained transition.

`scripts/issue350_runtime_v2_transition.py` is authoritative for the control-plane transition. This runbook explains its inputs and acceptance rules; operators must not replace it with hand-entered mutation commands. Its only permitted mutations are:
...
The retained-transition path and destructive clean-bootstrap path below are **mutually exclusive**. `--mode retained-transition` is only for the existing old/old retained cluster and performs the three CAS mutations above. It must never be run on a fresh cluster. `--mode clean-bootstrap-accept` is post-`up` acceptance for a newly recreated empty `digiorg-core-dev`; it performs only bounded reads and never executes a transition patch.
...
- v2 tag: `issue350-runtime-v2-20260902T063718Z`
- product base: `1190d6c26364437511e69091c44352ef4aedd4b6`
- v2 runtime commit: the 40-character SHA produced only after exact-snapshot PASS
- core-catalog: `d531180b322dc0128477ecb9bb0fc9071b41d631`

The final 32 Core repository source references are exactly three on v2 and 29 on the old tag: Root `apps`, Argo CD `platform/base/argocd`, and the OpenSearch `ref: values` source are v2; OpenSearch `path: platform/base/opensearch`, Fluentd, and every other Core source remain old. The exact 32-Application inventory is `app-config,argocd,backstage,cert-manager,cnpg,cnpg-cluster,core-catalog,crossplane,crossplane-harbor-bootstrap,crossplane-provider-configs,crossplane-providers,crossplane-xrds,external-secrets,fluentd,gitea,gitea-actions-runner,grafana,harbor,jaeger,keycloak,kyverno,kyverno-policies,landingpage,monitoring-extras,namespaces,nats,nats-jetstream-controller,opencost,opensearch,postgresql,root-app,sonarqube`; missing or extra Applications fail before mutation. OpenSearch chart is exactly `3.7.0`. External app-config remains `https://digiorg.local/gitea/DigiOrg/app-config.git`, path `claims`, target `main`; core-catalog remains `https://github.com/digiorg/core-catalog.git`, path `compositions/local`, at its pinned SHA.
...
CHECKOUT_350=/secure/issue350-runtime-v2
...
python3 scripts/issue350_runtime_v2_transition.py \
...
- the annotated remote v2 tag peels to the supplied runtime commit;
- minified kubeconfig server and `Namespace/kube-system.metadata.uid` equal supplied values;
- every Argo CD Application has `spec.operation` absent, no `Running`/`Terminating` operation, and exact `Synced`/`Healthy` status;
...
- all complete in-memory Application specs, canonical hashes, resource versions/UIDs, Root/Argo/OpenSearch operation identities, Argo comparison identity, and app-config resolved revision are captured (only hashes/scalars reach evidence); every prior operation has nonempty valid RFC3339 `startedAt` and nonempty requested and syncResult revision lists; app-config has exactly one nonempty lowercase 40-hex resolved commit (arbitrary status text is rejected and never emitted);
...
- no HPA in any namespace targets that StatefulSet.
...
Acceptance requires all Application UIDs unchanged and every complete Application spec equal to preflight except exactly Root target→v2, Argo target→v2, and OpenSearch values-ref target→v2. All other fields across the root and all 31 child Applications remain exact. All Applications also have no requested/active operation and are `Synced`/`Healthy`, plus:

- Root target is v2, status sync revision is the runtime commit, and its new `Succeeded` operation has valid RFC3339 `startedAt` chronologically later than preflight; both requested and syncResult revision lists equal `[runtime commit]` exactly;
- Argo CD is deliberately zero-diff: no sync operation is required, but its post-restoration `reconciledAt` is valid RFC3339 and chronologically later than the captured pre-restore value; target/status revision are v2/runtime commit;
- OpenSearch has exact source identities/order (chart repo+chart `opensearch`+`3.7.0`; Core repo `ref: values` with no path at v2; Core repo `path: platform/base/opensearch` with no ref at old), exact status revisions, and a new `Succeeded` operation whose requested and syncResult revision lists both equal `[3.7.0, runtime commit, old commit]` and whose RFC3339 `startedAt` is chronologically later;
...
- all 32 Core refs are exactly 3 v2 / 29 old / 0 other;
...
## Deterministic clean bootstrap

This is the authorized destructive recreation path. The commands are literal and contain no operator-substitution fields. They first remove only `digiorg-core-dev`, prove it absent, clone and detach the immutable annotated mixed tag into a new path, prove the exact checkout clean, and run the repository's existing `nu scripts/local-setup.nu up` as the only bootstrap mutation. The acceptance mode is read-only after `up`. Do not run the retained-transition mode before or after this path.

```bash
kind delete cluster --name digiorg-core-dev
test -z "$(kind get clusters | grep -Fx digiorg-core-dev || true)"
test ! -e /home/simonremote/issue350-clean-bootstrap
git clone --no-checkout https://github.com/digiorg/core.git /home/simonremote/issue350-clean-bootstrap
cd /home/simonremote/issue350-clean-bootstrap
git fetch --force origin refs/tags/issue350-runtime-v2-20260902T063718Z:refs/tags/issue350-runtime-v2-20260902T063718Z
git checkout --detach "refs/tags/issue350-runtime-v2-20260902T063718Z^{}"
test -z "$(git status --porcelain=v1 --untracked-files=all)"
RUNTIME_COMMIT="$(git rev-parse HEAD)"
test "$(git rev-parse 'refs/tags/issue350-runtime-v2-20260902T063718Z^{}')" = "$RUNTIME_COMMIT"
nu scripts/local-setup.nu up
chmod 0600 /home/simonremote/issue350-clean-bootstrap/kubeconfig-local.yaml
python3 scripts/issue350_runtime_v2_transition.py \
  --mode clean-bootstrap-accept \
  --kubeconfig /home/simonremote/issue350-clean-bootstrap/kubeconfig-local.yaml \
  --context kind-digiorg-core-dev \
  --remote-url https://github.com/digiorg/core.git \
  --runtime-tag issue350-runtime-v2-20260902T063718Z \
  --runtime-commit "$RUNTIME_COMMIT" \
  --evidence /home/simonremote/issue350-clean-bootstrap-accept.jsonl
```
